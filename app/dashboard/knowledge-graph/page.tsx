...
import { graphForSelection, normalizeGraphText } from "../../lib/knowledge-graph-model";
...
export default function KnowledgeGraphPage() {
  ...
  const requestedAt = atParam ? Date.parse(atParam) : NaN, requestedFrom = fromParam ? Date.parse(fromParam) : NaN, requestedTo = toParam ? Date.parse(toParam) : NaN;
  const dates = data.graph?.dates || [];
  const latestAt = dates.at(-1) ?? 0;
  const selection = useMemo<GraphSelection>(() => {
    if (modeParam === "all") return { mode: "all" };
    if (modeParam === "range" && Number.isFinite(requestedFrom) && Number.isFinite(requestedTo)) return { mode: "range", from: requestedFrom, to: requestedTo };
    return { mode: "latest", at: Number.isFinite(requestedAt) ? requestedAt : latestAt };
  }, [modeParam, requestedAt, requestedFrom, requestedTo, latestAt]);
  ...
  return <main className="dashboard-shell kg-shell">
    ...
    {!access.initialized ? <DetailSkeleton /> : !access.token ? <section className="token-gate"><div className="token-gate-copy"><p className="eyebrow">Your knowledge</p><h1>See the connections.</h1><p>Sign in to explore what your projects have learned.</p></div><AccessTokenForm desktop={desktop} loading={false} value={access.tokenInput} onChange={access.setTokenInput} error={access.authError} onSubmit={access.signIn} /></section> : !supported ? <SessionEmpty title="A new way to see your work." description="Update & restart in App settings to explore knowledge graphs." icon="refresh" /> : catalog.error ? <GraphError error={catalog.error} retry={catalog.refresh} /> : !catalog.runs ? <div className="session-main"><DetailSkeleton /><p role="status">Finding your knowledge graphs…</p></div> : !projects.length ? null : requestedProject && !projects.some(([id]) => id === project) ? <SessionEmpty title="This project is unavailable." description="Choose an accessible project to explore its knowledge." action={<button className="session-button" onClick={() => update({ project: projects[0][0], session: [], source: "", mode: "latest", at: "", from: "", to: "", node: "" })}>Choose another project</button>} /> : <div className="kg-main">
      ...
      <div className="kg-toolbar">
        <label>Project<select aria-label="Project" value={project} onChange={event => update({ project: event.target.value, session: [], source: "", mode: "latest", at: "", from: "", to: "", node: "", q: "", edge_q: "", relationship: "" })}>{projects.map(([id, name]) => <option key={id} value={id}>{projectName(name)}</option>)}</select></label>
        <label>Source<select aria-label="Source" value={source} onChange={event => update({ source: event.target.value, mode: "latest", at: "", from: "", to: "", node: "" })}><option value="">All sources</option>{sources.map(item => <option key={item} value={item}>{sourceName(item)}</option>)}</select></label>
        <details className="kg-session-filter"><summary><SlidersHorizontal size={14} />{filters.sessions.length ? `${filters.sessions.length} selected sessions` : "All sessions"}</summary><div><button className="session-text-link" onClick={() => update({ session: [], mode: "latest", at: "", from: "", to: "", node: "" })}>All sessions</button>{sessions.map(([id, name]) => <label key={id}><input type="checkbox" checked={filters.sessions.includes(id)} onChange={event => update({ session: event.target.checked ? [...filters.sessions, id] : filters.sessions.filter(item => item !== id), mode: "latest", at: "", from: "", to: "", node: "" })} />{name}</label>)}</div></details>
