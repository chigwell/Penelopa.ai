...
export function resolveGraphSelection(input: { mode?: string | null; at?: string | null; from?: string | null; to?: string | null }, latestAt: number): GraphSelection {
  const requestedAt = input.at ? Date.parse(input.at) : NaN;
  const requestedFrom = input.from ? Date.parse(input.from) : NaN;
  const requestedTo = input.to ? Date.parse(input.to) : NaN;
  if (input.mode === "range" && Number.isFinite(requestedFrom) && Number.isFinite(requestedTo)) return { mode: "range", from: requestedFrom, to: requestedTo };
  if (input.mode === "latest" || !input.mode && input.at) return { mode: "latest", at: Number.isFinite(requestedAt) ? requestedAt : latestAt };
  return { mode: "all" };
}
export function mergeKnowledgeGraphs(snapshots: GraphSnapshot[], filters: GraphFilters): KnowledgeGraph {
  ...
  for (const { snapshot: { run, nodes: rawNodes, edges: rawEdges }, at } of ordered) {
    ...
    const origin = (record: GraphRecord, label: string): GraphOrigin => ({
      runId: run.id, projectId: run.project_id, projectKey: run.project_key, sessionId: run.session_id, sessionKey: run.session_key, source: run.source, at,
    ...
    for (const raw of rawNodes) {
      ...
      else { nodes.set(id, { id, projectId: run.project_id, projectKey: run.project_key, label, firstSeen: at, observedAt: at, origins: [origin(raw, label)], search: searchable(raw) }); dates.add(at); }
    }
    for (const raw of rawEdges) {
      ...
      else { edges.set(id, { id, projectId: run.project_id, projectKey: run.project_key, source, target, relationship, firstSeen: at, observedAt: at, origins: [origin(raw, relationship)], search: searchable(raw) }); dates.add(at); }
    }
  }
