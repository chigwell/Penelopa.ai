...
function render(state) {
  ...
  document
    ...
    .forEach((element) =>
      element.classList.toggle("active", element.dataset.page === state.page),
    );
  document.getElementById("version").textContent =
    `Penelopa.ai · ${state.version}`;
  const connected = state.connection.agents?.some(
    (agent) => agent.state === "connected",
  );
  document
    .getElementById("connection-dot")
    .classList.toggle("connected", connected && !state.preferences.paused);
  document.getElementById("connection-label").textContent = state.preferences
    .paused
    ? "Collection paused"
    : connected
      ? "Hooks connected"
  ...
  document.getElementById("page-title").textContent =
    ...
    }[state.page] || "Your workspace";
  document.getElementById("content").innerHTML =
    state.page === "connection"
      ? connection(state)
      : state.page === "settings"
        ? settingsPage(state)
        : state.page === "offline"
...
document.addEventListener("click", (event) => {
  ...
  if (!element) return;
  if (element.dataset.page) void invoke("navigate", element.dataset.page);
  else if (element.dataset.action) void invoke(element.dataset.action);
});
document.addEventListener("change", (event) => {
  const input = event.target;
  if (input.dataset.setting)
    void invoke("preferences", { [input.dataset.setting]: input.checked });
});
if (bridge) {
  bridge.onState((state) => {
    const changedError =
      state.nativeError && state.nativeError !== current?.nativeError;
    render(state);
    if (changedError) message(state.nativeError);
  });
  bridge
    .invoke("state")
    .then(render)
    .catch(() => message("Connection status could not be loaded."));
}
